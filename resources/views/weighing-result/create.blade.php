...
@section('title', 'Create new internship-request')
...
@section('content')
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('internship-request.store') }}" method="POST" enctype="multipart/form-data" class="w-full">
            @csrf
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select
                                ...
                                @foreach ($users as $item)
                                    <option value="{{$item->id}}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">{{$item->fullname}} &mdash; {{$item->student->major->name}}</option>
                                @endforeach
                            </select>
                            <span
                            ...
                            <select
                                ...
                                @foreach ($companies as $item)
                                    <option value="{{ $item->id }}" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
                                        {{ $item->name }} &mdash;
                                        {!! str_repeat('★', min($item->reputations, 5)) . str_repeat('☆', 5 - min($item->reputations, 5)) !!} ({{$item->reputations}}), Max. {{$item->max_capacity}} Person
                                    </option>
                                @endforeach
                            </select>
                            <span
