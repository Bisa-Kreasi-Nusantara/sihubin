...
@section('content')
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <form action="{{ route('users-management.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="w-full">
            @csrf
            @method('PUT')
                        ...
                        <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                            <select
                                ...
                                @foreach ($roles as $item)
                                    <option value="{{$item->id}}" {{ $item->id == $user->roles_id ? 'selected' : '' }} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">{{$item->name}}</option>
                                @endforeach
                            </select>
                            <span
